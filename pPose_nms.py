...
def p_merge_fast(ref_pose, cluster_preds, cluster_scores, ref_dist):
    ...
    # Weighted Merge
    masked_scores = cluster_scores.cuda().mul(mask.float().unsqueeze(-1).cuda())
    normed_scores = masked_scores / torch.sum(masked_scores, dim=0)
    masked_scores = masked_scores.cpu()
    normed_scores = normed_scores.cpu()
...
def get_parametric_distance(i, all_preds, keypoint_scores, ref_dist):
    ...
    final_dist = torch.sum(score_dists, dim=1).cuda() + mu * torch.sum(point_dist, dim=1).cuda()
    final_dist = final_dist.cpu()
